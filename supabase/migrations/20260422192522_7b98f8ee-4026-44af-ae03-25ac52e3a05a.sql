ALTER TABLE public.user_books ADD COLUMN IF NOT EXISTS url text NOT NULL DEFAULT '';
...
BEGIN
  ...
  IF share_row.tab = 'courses' THEN
    SELECT jsonb_build_object(
      'share', jsonb_build_object('id', share_row.id, 'name', share_row.name, 'tab', share_row.tab, 'filters', share_row.filters, 'view_mode', f_view_mode),
    ...
    FROM (
      ...
      WHERE user_id = share_row.user_id
        ...
        AND (f_search IS NULL OR f_search = '' OR title ILIKE '%' || f_search || '%' OR platform ILIKE '%' || f_search || '%' OR instructor ILIKE '%' || f_search || '%')
  ...
  ELSE
    ...
    INTO result
    FROM (
      SELECT id, title, author, total_pages, pages_read, rating, status, notes, cover_color, tags, pillars, directions, format, url, created_at, updated_at
      ...
      WHERE user_id = share_row.user_id
        ...
        AND (f_pillar IS NULL OR f_pillar = '' OR f_pillar = ANY(pillars))
        AND (f_search IS NULL OR f_search = '' OR title ILIKE '%' || f_search || '%' OR author ILIKE '%' || f_search || '%')
      ORDER BY created_at DESC
    ) b;
  END IF;
