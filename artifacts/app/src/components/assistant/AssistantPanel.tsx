      ...
      <AnimatePresence>
        {!open && (
          <motion.button
            ...
            className={
              isWatch
                ? "fixed bottom-2 right-2 z-[9998] flex items-center justify-center w-8 h-8 rounded-full gradient-purple text-primary-foreground glow-md hover:-translate-y-0.5 transition-transform"
                : "fixed bottom-5 right-5 z-[9998] flex items-center gap-2 px-4 py-3 rounded-2xl gradient-purple text-primary-foreground font-bold glow-md hover:-translate-y-0.5 transition-transform"
            }
            title="Ask the assistant"
          >
            <Sparkles className={isWatch ? "w-3.5 h-3.5" : "w-5 h-5"} />
            {!isWatch && <span className="hidden sm:inline text-sm">Assistant</span>}
          </motion.button>
        )}
      </AnimatePresence>
            ...
            >
              ...
              <div className={`flex items-center justify-between gap-1.5 border-b border-white/10 ${isWatch ? "px-2 py-1.5" : "px-4 py-3"}`}>
                <div className="flex items-center gap-1.5 min-w-0">
                  <div className={`rounded-xl flex items-center justify-center bg-primary/15 border border-primary/25 flex-shrink-0 ${isWatch ? "w-5 h-5 rounded-lg" : "w-8 h-8"}`}>
                    <Sparkles className={isWatch ? "w-3 h-3 text-primary" : "w-4 h-4 text-primary"} />
                  </div>
                  {!isWatch && (
              ...
              <div ref={listRef} className={`flex-1 overflow-y-auto space-y-3 ${isWatch ? "px-2 py-2" : "px-4 py-4 space-y-4"}`}>
                {messages.length === 0 && (
                  <div className={isWatch ? "text-center py-4" : "text-center py-10"}>
                    <div className={`mx-auto rounded-2xl flex items-center justify-center bg-primary/10 border border-primary/20 ${isWatch ? "w-8 h-8 mb-2" : "w-12 h-12 mb-3"}`}>
                      <Sparkles className={isWatch ? "w-4 h-4 text-primary" : "w-6 h-6 text-primary"} />
                    </div>
