...
- **Energy phases** is the slim Gather → Strike → Reflect bar above the category grid (`EnergyPhaseBar.tsx`, definitions in `lib/energy-phases.ts`). It is a per-day, per-device note of which beat you are in, so it lives in `localStorage` (`useEnergyPhase`) rather than Supabase, and resets each day to the hour-appropriate phase. Toggle it in Settings → Modules (`preferences.showEnergyPhases`, default on).
