...
export default function BookCard({ book, index, onClick }: BookCardProps) {
  ...
  const bookDirections = DIRECTIONS.filter(d => (book.directions || []).includes(d.id));
    ...
    >
      <div className="relative glass-card rounded-2xl overflow-hidden border border-white/5 hover:border-white/15 transition-all hover:-translate-y-1 hover:shadow-lg">
        ...
        <div className="pl-5 pr-4 py-4 flex gap-3">
          ...
          {/* Right: pillars & directions */}
          {(bookPillars.length > 0 || bookDirections.length > 0) && (
            <div className="flex flex-col gap-1 shrink-0 items-end">
              {bookPillars.map(p => (
                <span key={p.id} className="text-base leading-none" title={p.name}>{p.icon}</span>
              ))}
              {bookDirections.map(d => (
                <span key={d.id} className="text-base leading-none" title={d.label}>{d.icon}</span>
              ))}
            </div>
          )}
        </div>
