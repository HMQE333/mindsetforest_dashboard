      ...
      <div className="flex gap-2 items-center flex-wrap">
        ...
        <div className="flex gap-1">
          {["all", "tried", "want-to-try", "favourite", "failed"].map(s => (
            <button key={s} onClick={() => setFilterStatus(s)} className={`px-2.5 py-1.5 rounded-lg text-xs font-medium transition-all capitalize ${filterStatus === s ? "bg-primary/20 border border-primary/40 text-primary" : "bg-muted/20 border border-white/5 text-muted-foreground hover:border-white/20"}`}>
